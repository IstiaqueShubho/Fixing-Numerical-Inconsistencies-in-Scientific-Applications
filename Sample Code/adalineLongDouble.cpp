...
struct adaline
{
    long double eta;      /**< learning rate of the algorithm */
    long double *weights; /**< weights of the neural network */
...
struct adaline new_adaline(const int num_features, const double eta)
{
    ...
    ada.weights = (long double *)malloc(num_weights * sizeof(long double));
    if (!ada.weights)
    {
...
void adaline_fit(struct adaline *ada, double **X, const int *y, const int N)
{
         ...
         iter++)
    {
        ...
        for (int i = 0; i < N; i++)
        {
            long double err = adaline_fit_sample(ada, X[i], y[i]);
...
void test3(double eta)
{
    ...
    for (int i = 0; i < N; i++)
    {
        long double x0 = ((rand() % range) - range2) / 100.f;
        long double x1 = ((rand() % range) - range2) / 100.f;
        long double x2 = ((rand() % range) - range2) / 100.f;
        X[i][0] = x0;
        X[i][1] = x1;
        X[i][2] = x2;
        X[i][3] = x0 * x0;
